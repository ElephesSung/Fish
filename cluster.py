...
path = r"./KillFrequency28.8.25.txt"

try:
    with open(path, newline='', encoding='utf-8') as f:
        reader = list(csv.reader(f, delimiter='\t'))
except UnicodeDecodeError:
    with open(path, newline='', encoding='latin1') as f:  # or 'cp1252'
        reader = list(csv.reader(f, delimiter='\t'))
raw_header1 = reader[0]
raw_header2 = reader[1]
if raw_header1[0].strip() == "" and raw_header2[0].strip() == "":
    header1 = raw_header1[1:]
    header2 = raw_header2[1:]
else:
    header1 = raw_header1
    header2 = raw_header2
cond_names = []
...
for h in header1:
    ...
    if h_str:
        last = h_str
    cond_names.append(last)
if len(cond_names) != len(header2):
    raise ValueError("Header lengths mismatch after carry-forward: %d vs %d" % (len(cond_names), len(header2)))
KillFreqTable = defaultdict(lambda: defaultdict(list))
for row in reader[2:]:
    if not row:
        continue
    donor_val = int(row[0])
    cells = row[1:]
    if len(cells) < len(header2):
        cells += [""] * (len(header2) - len(cells))
    for i, cell in enumerate(cells):
        cnt = int(round(float(cell))) if str(cell).strip() else 0
        if cnt:
            cond = cond_names[i]
            date = header2[i].strip()
            KillFreqTable[cond][date].extend([donor_val] * cnt)
KillFreqTable = {
    cond: {date: np.array(vals, dtype=int) for date, vals in dates.items()}
    for cond, dates in KillFreqTable.items()
}
